...
const AlertMain: AlertMainComponent = forwardRef(function AlertMain<C extends React.ElementType = "div">(
  ...
  const { asTag, statusCode, hasIcon, className = "", children, ...restProps } = props
  ...
  return (
    ...
    >
      {hasIcon && (
        <AlertMainStatus>
          {statusCode === AlertStatusCode.Success && <Icon name="CheckCircleFilled" />}
          {statusCode === AlertStatusCode.Info && <Icon name="InfoCircleFilled" />}
          {statusCode === AlertStatusCode.Warning && <Icon name="ExclamationCircleFilled" />}
          {statusCode === AlertStatusCode.Error && <Icon name="CloseCircleFilled" />}
        </AlertMainStatus>
      )}
