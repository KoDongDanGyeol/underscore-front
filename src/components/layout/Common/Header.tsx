  ...
  return (
    <CommonHeaderContainer className={`${className}`} {...restProps}>
      <CommonHeaderButton
        ...
        size="lg"
        variants="secondary"
        iconEl={(props: Record<string, string>) => <Icon name="Menu" label="메뉴 (열기)" {...props} />}
        aria-haspopup="menu"
      ...
      <CommonHeaderUtils>
        {domain === "/map" && (
          <CommonHeaderButton
            ...
            size="lg"
            variants="secondary"
            iconEl={(props: Record<string, string>) => (
              <Icon name="FundProjectionScreen" label="지도 분석 (열기)" {...props} />
            )}
            aria-haspopup={true}
            aria-expanded={!!bottomSheetStructure.get("LocationSheet")}
            onClick={() => {
              bottomSheetStructure.get("LocationSheet") ? onUnmount("LocationSheet") : onMount("LocationSheet", {})
            }}
          />
        )}
      </CommonHeaderUtils>
