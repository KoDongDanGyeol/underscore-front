...
const useBottomSheet = () => {
  ...
  const onOpen = async (name: TypeName) => {
    ...
    onInput(name, { isPending: true, isOpened: false, isSpread: false })
    await setTimer(timers, { key: "delay", delay: 50 })
    onInput(name, { isPending: false, isOpened: true, isSpread: false })
    clearTimer(timers, { key: "delay" })
  }

  const onFold = async (name: TypeName) => {
    if (timers.current.delay) return
    onInput(name, { isPending: true, isOpened: true, isSpread: false })
    await setTimer(timers, { key: "delay", delay: 200 })
    onInput(name, { isPending: false, isOpened: false, isSpread: false })
    clearTimer(timers, { key: "delay" })
  }
